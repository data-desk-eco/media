# Media Mentions Tracker

Be fast. No commentary.

## Context

Data Desk investigates environmental/climate issues: oil/gas, fossil fuels, pollution, corporate accountability, green financing.

## Task

1. Read `data/curated.json` (never modify)
2. Read `data/discoveries.json` (your additions)
3. Read `data/search-results.json` (new Google News results)
4. Add relevant new articles to discoveries (dedupe by URL)
5. Write to `data/discoveries.json`

## Relevance

**Exclude:** "The Trade Desk", "Opportunity Desk", "CBA Data Desk", "Tiny Desk", hotel/service desks, trading desks, generic "data desk" mentions, thecradle.co. Also exclude NGO publications (e.g. Oil Change International, SourceMaterial, Pulitzer Center) — only media outlets republishing or citing the work belong here.

**Include:** Articles from media outlets we work with (Guardian, Examination, Bloomberg, Reuters, etc) that cite Data Desk environmental research.

When unsure, exclude.

## Output

```json
[{"title": "...", "url": "...", "source": "...", "published": "YYYY-MM-DD", "added": "ISO timestamp"}, ...]
```
